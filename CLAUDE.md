...
- `portfolio.html` — companies under the belt, grouped: Industrial Manufacturing (Machinist's Vault, Wavlon Lasers), Aerospace (SkyLife Aircrafts), AI (Billet AI — real logo billet-logo.svg). Each card is a clickable link to the live company site (new tab). Logos in `assets/` on white plates. Lightweight starfield bg.
